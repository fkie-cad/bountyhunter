...
Bounty Hunter offers various configuration parameters, e.g., "locking" abilities, reward updates, and final abilities, to customize the emulated attack behavior (see section "Bounty Hunter Configuration" below).
...
Then, two examples are introduced that show how Bounty Hunter can be used and what it is capable of.
The first example demonstrates Bounty Hunter's initial access and privilege escalation capabilities and can be used as a guide on how to use it.
The second example shows the high level of complexity of cyberattacks Bounty Hunter can emulate.
Finally, a more detailed description of how Bounty Hunter works, how it can be configured, and how it could be extended is given.
...
- Download the plugin
- Copy the `bountyhunter` directory into `caldera/plugins` and enable the plugin in the Caldera server's configuration (`caldera/conf/<config>.yml`)
- Add the following lines to the `caldera/Dockerfile` to install Bounty Hunter requirements during the docker build process, e.g., at line 77 after the installation of the emu plugins requirements in lines 69-76 
```
...
# Emulating complete, realistic cyberattacks with Bounty Hunter

The following two sections describe two examples how Bounty Hunter can be used and what it is capable of.
The first example shows how the initial access and privilege escalation capabilities of Bounty Hunter can be tested using demo adversaries and abilities.
In the second section an example attack based on an APT29 campaign is presented that shows the high level of complexity of cyberattacks Bounty Hunter can emulate.

## Scenario #1 - Initial Access and Privilege Escalation
The following section describes how to emulate a complete, realistic cyberattack chain using Bounty Hunter and can be used as a guide for getting started with it.
...
To run this scenario, set the scenario in Bounty Hunter's configuration (`data/planners/e1bb9388-1845-495d-b67b-ad61a31ff6cd.yml`) to `demo_initial_access_and_priv_esc` . 
...
1. Configure fact `bountyhunter.ip_range`: Using the Caldera UI, configure the IP address range Bounty Hunter should scan initially.
...
We note that the attack steps are NOT part of this plugin.
...
The following section go into more detail about how Bounty Hunter works, how it can be configured, and how it could be extended.
...
Which scenario Bounty Hunter should use can be configured in its configuration file (`bountyhunter/data/planners/e1bb9388-1845-495d-b67b-ad61a31ff6cd.yml`).

The following table lists the various parameters used by Bounty Hunter including a short description and the default values.
...
How far, i.e., how many abilities ahead, Bounty Hunter uses ability rewards is controlled by the depth parameter while the discount factor controls how much influence future ability reward values have on the calculation.
...
- [Blog Post](https://lolcads.github.io/posts/2024/09/bountyhunter/) that introduces Bounty Hunter and gives some information about adversary emulation, its challenges, and Caldera.
